...
func main() {
	...
	w, h := size.X, size.Y
	result := image.NewNRGBA64(image.Rect(0, 0, w, h))
	for y := 0; y < h; y++ {
		for x := 0; x < w; x++ {
			...
			r := uint16(math.Min(65535, fr*65535))
			g := uint16(math.Min(65535, fg*65535))
			b := uint16(math.Min(65535, fb*65535))
			result.SetNRGBA64(x, y, color.NRGBA64{r, g, b, 65535})
		}
	}
