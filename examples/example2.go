...
func main() {
	scene := pt.Scene{}
	material := pt.Material{pt.HexColor(0xEFC94C), 3, pt.Radians(30), 0}
	whiteMat := pt.Material{pt.Color{1, 1, 1}, 3, pt.Radians(30), 0}
	for x := 0; x < 10; x++ {
		for z := 0; z < 10; z++ {
			center := pt.Vector{float64(x) - 4.5, 0, float64(z) - 4.5}
			scene.AddShape(pt.NewSphere(center, 0.4, material, nil))
		}
	}
